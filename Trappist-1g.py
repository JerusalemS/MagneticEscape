...
rg_exo = R_G + (396000 *u.m) 
M_G = 1.321 * M_earth.to(u.kg)

ng_sw = 1.95e3 *(1/u.cm**3)      # * 1e6 convert cm^-3 to m^-3   # DIFFERENT (USED EARTH PARAMS)
vg_sw = 637000  * u.m/u.s                               # DIFFERENT (USED EARTH PARAMS)
prot_mass = 1.67e-27 *u.kg

Omega_pc_E = 0.63
r_exo_E = 6871000 *u.m
rg_IMB = 7647000  *u.m                           # DIFFERENT (USED 6666800 m)
formfact_o = 1.16
...
Q0_pu_H = 5e26 *( 1/u.s) 
hg_H = (k_B * T_exo_H * rg_exo**2) / (G * M_G * mass)

PU_escape = []
pickup_vals = []
for M in magnetic_moments:
    R_MP = ((magnetic_p * M**2) / (8 * np.pi**2 * ng_sw * vg_sw**2 * prot_mass))**(1/6)
    if R_MP <= rg_exo:
        Q_pu = Q0_pu_H
    else:
        num = 2*hg_H**3 + 2*hg_H**2*R_MP + hg_H*R_MP**2
        den = 2*hg_H**3 + 2*hg_H**2*rg_exo + hg_H*rg_exo**2
        exp_term = np.exp((rg_exo - R_MP) / hg_H)
...
pc_vals = []
for M in magnetic_moments:
    R_MP = (((magnetic_p * formfact_o**2 * M**2) / (8 * np.pi**2 * ng_sw * vg_sw**2 * prot_mass))**(1/6))
    if R_MP > rg_IMB:
        temp = 1 - (r_exo_E / R_MP)
        Omega_pc = 4 * np.pi * (1 - np.sqrt(temp)) if temp >= 0 else 0
    else:
        Omega_pc = 0
    Q_pc = Q0_pc_H * (Omega_pc / Omega_pc_E) * ((rg_exo / r_exo_E)**2)
    pc_vals.append(Q_pc)
...
for M in magnetic_moments:
    ...
    if R_MP > rg_IMB:
        ...
        Omega_pc = 0
    Q_cu = min(Q0_cu_H * (r_c_g / r_c_E)**2, Qmax_cu_H) * (Omega_pc / Omega_pc_E) * (rg_exo / r_exo_E)**2
...
for M in magnetic_moments:
    r_mp = (((magnetic_p * M**2) / (8 * np.pi**2 * ng_sw * vg_sw**2 * prot_mass))**(1/6))  
    h_exo_g = (k_B * T_exo_H * rg_exo**2) / (G * M_G * m_H)
    n_exo_g = (1 / (h_exo_g * sigma_coll))  #.to(1 / u.m**3)
    n_L_g = n_exo_g * np.exp((-rg_exo / h_exo_g) * (1 - (rg_exo / r_mp)))
    surface_a_g = (4 * np.pi * r_mp**2)
    f_lm_g = (((energy_eff * h_exo_g * surface_a_g * (n_L_g - ng_sw)) / time) * mass_water) #.decompose()
    magnetic_limited_vals.append(f_lm_g)

for f_lm_g in magnetic_limited_vals:
    mass_loss_rate =  f_lm_g * mass          # Multiply by mass of hydrogen to get units in kg/s
    mass_loss_rate_atmospheres = (mass_loss_rate / earth_atmosphere_in_kg) * seconds_per_gyr  
...
a = (84.591 * 84e6) * (u.m)     # Used ratio from Eric Agol paper ( a/R*)
M_star = 0.09*M_sun
...
rho = M_G / ((4/3) * np.pi * R_G**3)                    

# Roche lobe radius (Becker+2020 uses 2 in denominator!)
r_roche = a * (M_G / (2 * M_star))**(1/3)

# Tidal enhancement factor K
K = 1 - (3/2) * (R_G / r_roche) + (1/2) * (R_G / r_roche)**3

# Energy-limited escape rate (mass loss) in kg/s
mdot = epsilon * (R_G / R_G)**2 * (3 * F_xuv) / (4 * G * rho * K)
