...
R_H = 0.755 * R_earth.to(u.m)
rh_exo = R_H + (396000 *u.m) 
M_H = 0.326 * M_earth.to(u.kg)

nh_sw = 9.52e2 *(1/u.cm**3)      # * 1e6 convert cm^-3 to m^-3   # DIFFERENT (USED EARTH PARAMS)
vh_sw = 657000  * u.m/u.s                               # DIFFERENT (USED EARTH PARAMS)
prot_mass = 1.67e-27 *u.kg

Omega_pc_E = 0.63
r_exo_E = 6871000 *u.m
rh_IMB = 7647000  *u.m                           # DIFFERENT (USED 6666800 m)
formfact_o = 1.16
...
Q0_pu_H = 5e26 *( 1/u.s) 
hH_H = (k_B * T_exo_H * rh_exo**2) / (G * M_H * mass)

PU_escape = []
pickup_vals = []
for M in magnetic_moments:
    R_MP = ((magnetic_p * M**2) / (8 * np.pi**2 * nh_sw * vh_sw**2 * prot_mass))**(1/6)
    if R_MP <= rh_exo:
        Q_pu = Q0_pu_H
    else:
        num = 2*hH_H**3 + 2*hH_H**2*R_MP + hH_H*R_MP**2
        den = 2*hH_H**3 + 2*hH_H**2*rh_exo + hH_H*rh_exo**2
        exp_term = np.exp((rh_exo - R_MP) / hH_H)
...
pc_vals = []
for M in magnetic_moments:
    R_MP = (((magnetic_p * formfact_o**2 * M**2) / (8 * np.pi**2 * nh_sw * vh_sw**2 * prot_mass))**(1/6))
    if R_MP > rh_IMB:
        temp = 1 - (r_exo_E / R_MP)
        Omega_pc = 4 * np.pi * (1 - np.sqrt(temp)) if temp >= 0 else 0
    else:
        Omega_pc = 0
    Q_pc = Q0_pc_H * (Omega_pc / Omega_pc_E) * ((rh_exo / r_exo_E)**2)
    pc_vals.append(Q_pc)
...
for M in magnetic_moments:
    ...
    if R_MP > rh_IMB:
        ...
        Omega_pc = 0
    Q_cu = min(Q0_cu_H * (r_c_h / r_c_E)**2, Qmax_cu_H) * (Omega_pc / Omega_pc_E) * (rh_exo / r_exo_E)**2
...
for M in magnetic_moments:
    r_mp = (((magnetic_p * M**2) / (8 * np.pi**2 * nh_sw * vh_sw**2 * prot_mass))**(1/6))  
    h_exo_h = (k_B * T_exo_H * rh_exo**2) / (G * M_H * mass)
    n_exo_h = (1 / (h_exo_h * sigma_coll))  #.to(1 / u.m**3)
    n_L_h = n_exo_h * np.exp((-rh_exo / h_exo_h) * (1 - (rh_exo / r_mp)))
    surface_a_h = (4 * np.pi * r_mp**2)
    f_lm_h = (((energy_eff * h_exo_h * surface_a_h * (n_L_h - nh_sw)) / time) * mass_water) #.decompose()
    magnetic_limited_vals.append(f_lm_h)

for f_lm_h in magnetic_limited_vals:
    mass_loss_rate =  f_lm_h * mass          # Multiply by mass of hydrogen to get units in kg/s
    mass_loss_rate_atmospheres = (mass_loss_rate / earth_atmosphere_in_kg) * seconds_per_gyr  
...
a = (111.817 * 84e6) * (u.m)     # Used ratio from Eric Agol paper ( a/R*)
M_star = 0.09*M_sun
...
rho = M_H / ((4/3) * np.pi * R_H**3)                    

# Roche lobe radius (Becker+2020 uses 2 in denominator!)
r_roche = a * (M_H / (2 * M_star))**(1/3)

# Tidal enhancement factor K
K = 1 - (3/2) * (R_H / r_roche) + (1/2) * (R_H / r_roche)**3

# Energy-limited escape rate (mass loss) in kg/s
mdot = epsilon * (R_H / R_H)**2 * (3 * F_xuv) / (4 * G * rho * K)
...
plt.loglog(magnetic_moments, mdot_array, label='Energy Limited Escape', color='red', linewidth=3)
...
Q0_pu_H = 5e26 *( 1/u.s) 
hH_H = (k_B * T_exo_H * rh_exo**2) / (G * M_H * m_H)
...
for M in magnetic_moments:
    r_mp = (((magnetic_p * M**2) / (8 * np.pi**2 * nh_sw * vh_sw**2 * prot_mass))**(1/6))  
    h_exo_h = (k_B * T_exo_H * rh_exo**2) / (G * M_H * m_H)
    n_exo_h = (1 / (h_exo_h * sigma_coll))  #.to(1 / u.m**3)
    n_L_h = n_exo_h * np.exp((-rh_exo / h_exo_h) * (1 - (rh_exo / r_mp)))
    surface_a_h = (4 * np.pi * r_mp**2)
    f_lm_h = (((energy_eff * h_exo_h * surface_a_h * (n_L_h - nh_sw)) / time) * mass_water) #.decompose()
